...
namespace UTAUotoReplacer
{
    public partial class FormMain : Form
    {
        ...
        private void GetOldValueLength()
        {
            int counter = 0;
            int replaceRows = dataGridView2.Rows.Count - 1;
            while (counter < replaceRows)
            {
                string s = GetDataGrid2(0, counter);
                int n = s.Length;
                dataGridView2[2, counter].Value = n;
                counter = counter + 1;
            }
        }
