...
namespace Replacer
{
    public partial class FormReplace : Form
    {
        ...
        public FormReplace(FormMain formMain)
        {
            frmMain = formMain;
            InitializeComponent();
        ...
        private void replaceTextToolStripMenuItem_Click(object sender, EventArgs e)
        {
            int counter = 0;
            int datarows = dataGridView1.Rows.Count - 1; //有 n 列取代條件
            string s = frmMain.richTextBox1.Text;
            while (counter < datarows)
            {
                s = s.Replace(dataGridView1[0, counter].Value.ToString(), dataGridView1[1, counter].Value.ToString());
                counter = counter + 1;
            }
            MessageBox.Show("Finished!");
            frmMain.richTextBox1.Text = s;
        }
    }
}
